...
def get_rates_percent_change():
    ...
    YESTERDAY_END = datetime.datetime(now.year, now.month, now.day, 0, 0, 0) - datetime.timedelta(seconds=1)
    ...
    if(yesterdayExchangeRate[0] != 0):
        changeUSDtoLBP = ((todayExchangeRate[0] - yesterdayExchangeRate[0]) / yesterdayExchangeRate[0]) * 100
    changeLBPtoUSD = "Not available : no LBP to USD transactions were done yesterday"
    if(yesterdayExchangeRate[1] != 0): 
        changeLBPtoUSD = ((todayExchangeRate[1] - yesterdayExchangeRate[1]) / yesterdayExchangeRate[1]) * 100
